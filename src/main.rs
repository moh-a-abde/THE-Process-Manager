...
fn build_ui(application: &Application) {
    ...
    button_all.connect_clicked({
        ...
        move |_| {
            ...
            for (i, process) in processes.iter().enumerate() {
                ...
                apply_color_style(&resident_mem_label, "white"); // Default color for Resident Memory
    ...
    // Set up the layout
    let vbox = gtk::Box::new(gtk::Orientation::Vertical, 5);
    vbox.pack_start(&button_pid, false, false, 5);
    vbox.pack_start(&button_all, false, false, 5);
    vbox.pack_start(&sort_button, false, false, 5);
    vbox.pack_start(&filter_state_button, false, false, 5);
